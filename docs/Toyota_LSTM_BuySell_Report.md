...
|:--|:--:|:--:|:--:|:--|
| **LightGBM** | **53.55%** | 162 | 148 | 最高精度 ⭐ |
| XGBoost | 53.23% | 156 | 154 | 高速 |
| RandomForest | 49.68% | 31 | 279 | ベースライン |
| LSTM | 48.71% | 0 | 310 | 時系列学習 |
| GRU | 48.39% | 0 | 310 | 時系列学習 |
...
| 順位 | 特徴量 | 重要度 | 解釈 |
|:--:|:--|:--:|:--|
| 1 | ma_20 | 0.133 | 20日移動平均が最重要 |
| 2 | ma_5 | 0.127 | 5日移動平均も重要 |
| 3 | ret_1d | 0.100 | 前日のリターン |
| 4 | macdhist | 0.099 | MACDヒストグラム |
| 5 | bb_width | 0.098 | ボラティリティ |
| 6 | macd | 0.093 | MACDトレンド |
| 7 | vol_change | 0.092 | 出来高変化 |
| 8 | macdsignal | 0.087 | MACDシグナル |
| 9 | ma_ratio | 0.086 | 移動平均比率 |
| 10 | rsi | 0.083 | RSI |

**分析**:
- 移動平均系の特徴量が最も重要（ma_20, ma_5で合計26%）
- MACD系特徴量も有効（合計28%）
- すべての特徴量が一定の寄与をしており、バランスが良い
...
| 閾値 | 正解率 | 下落予測 | 上昇予測 | バランス |
|:--:|:--:|:--:|:--:|:--:|
| 0.45 | 49.35% | 6 | 304 | 0.961 |
| 0.50 | 49.68% | 31 | 279 | 0.800 |
| 0.52 | 50.65% | 60 | 250 | 0.613 |
| 0.55 | 50.65% | 148 | 162 | 0.045 |
| **0.57** | **53.55%** | **197** | **113** | 0.271 |
| 0.59 | 51.61% | 247 | 63 | 0.594 |
...
### 取引戦略
- **買いシグナル**: モデルが上昇予測（確率≥0.57）の日に株を購入
- **待機**: モデルが下落予測の日は現金保有
...
正解率49%, 下落予測0%→10%
    ↓ RandomForest導入
正解率49.68%, 下落予測10%→19.4%
    ↓ 閾値最適化（0.52）
正解率50.65%, 下落予測19.4%→38.7%
    ↓ XGBoost/LightGBM導入
正解率53.23%, より高精度
    ↓ 閾値最適化（0.57）
正解率53.55%, バランス改善
...
2. **実用的な予測精度**
   - LightGBMで53.55%の正解率を達成
   - 単純な買い持ちを40.93%上回るリターン
